...
# Also used the transform.scale method and put the image as the first arg and the w and h as the secodn arg.
...
def main():
	...
	def redraw_window():
		...
		if lost:
			lost_label = lost_font.render("LOOOOOOOOOSER!", 1, (255, 255, 255))
			WIN.blit(lost_label, (WIDTH/2 - lost_label.get_width()/2, 300))

		pygame.display.update()

# This loop sets the time to run at 60 frames per second
	while run:
		Clock.tick(FPS)
		redraw_window()
...
# This event allows the user to quit the window. It checks for this 60 times per second.
